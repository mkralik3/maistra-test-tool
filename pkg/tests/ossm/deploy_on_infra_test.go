...
func TestDeployOnInfraNodes(t *testing.T) {
	test.NewTest(t).Id("T40").Groups(test.Full, test.Disconnected).Run(func(t test.TestHelper) {
		...
		t.Cleanup(func() {
			csvVersion := operator.GetCsvName(t, env.GetOperatorNamespace(), "servicemeshoperator")
			...
			operator.WaitForOperatorReady(t, env.GetOperatorNamespace(), "name=istio-operator", csvVersion)
		})
